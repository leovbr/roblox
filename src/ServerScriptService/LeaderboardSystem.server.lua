...
local function formatNumber(n)
	...
	local i = 1
	while math.abs(n) >= 1000 and i < #suffixes do n /= 1000; i += 1 end
...
local function makeBoard(name, title, position)
	...
	part.Name = name; part.Anchored = true; part.Size = Vector3.new(15,9,.7); part.Position = position
	part.Material = Enum.Material.SmoothPlastic; part.Parent = folder

	local gui = Instance.new("SurfaceGui")
	gui.Name="BoardGui"; gui.Face=Enum.NormalId.Front; gui.CanvasSize=Vector2.new(900,650); gui.Parent=part

	local titleLabel=Instance.new("TextLabel")
	titleLabel.BackgroundTransparency=1; titleLabel.Size=UDim2.new(1,0,0,100); titleLabel.Text=title
	titleLabel.Font=Enum.Font.GothamBlack; titleLabel.TextScaled=true; titleLabel.TextColor3=Color3.new(1,1,1); titleLabel.Parent=gui

	local body=Instance.new("TextLabel")
	body.Name="Entries"; body.BackgroundTransparency=1; body.Position=UDim2.fromOffset(25,105); body.Size=UDim2.new(1,-50,1,-125)
	body.TextXAlignment=Enum.TextXAlignment.Left; body.TextYAlignment=Enum.TextYAlignment.Top; body.Font=Enum.Font.GothamBold
	body.TextSize=30; body.TextColor3=Color3.new(1,1,1); body.Text="Loading..."; body.Parent=gui
	return body
end
...
local function readTop(store, formatter)
	...
	for rank,entry in ipairs(pages:GetCurrentPage()) do
		...
		lines[#lines+1]=string.format("%d. %s — %s",rank,nameForUserId(userId),formatter(entry.value))
	end
...
local function savePlayer(player)
	...
	activeSeconds[player]=0; savedActive[player]=seconds
	local data=player:FindFirstChild("GameData")
...
Players.PlayerAdded:Connect(function(player)
	...
	local ok,previous=pcall(function()return activeStore:GetAsync(tostring(player.UserId))end)
	savedActive[player]=ok and tonumber(previous) or 0
end)

Players.PlayerRemoving:Connect(function(player)
	savePlayer(player); activeSeconds[player]=nil; savedActive[player]=nil
end)

task.spawn(function()
	while true do
		task.wait(1)
		for _,player in ipairs(Players:GetPlayers()) do activeSeconds[player]=(activeSeconds[player] or 0)+1 end
	end
end)
