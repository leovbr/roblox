...
local function cashOf(player) local s=player:FindFirstChild("leaderstats"); return s and s:FindFirstChild("Cash") end
...
local function award(player, action)
	...
	if action:match("^Treadmill%d+$") then
		local level = tonumber(action:match("%d+$")); local speedLevel = data:FindFirstChild("SpeedLevel")
	...
	if action:match("^SpeedX%d+$") then
		local multiplier = tonumber(action:match("%d+$")); local boost = Config.SPEED_BOOSTS["X" .. multiplier]
		local speedMultiplier = data:FindFirstChild("SpeedMultiplier")
		if not boost or not speedMultiplier or multiplier <= speedMultiplier.Value then return false end
	...
	if action == "SpeedInstant1M" then
		...
		-- Robux purchase is the alternative to paying the $1M cash price.
		speedLevel.Value += 1
		return true
	end
	...
	if action:match("^Trail") then
		...
		owned.Name = "OwnedTrails"; owned.Parent = data
		local flag = owned:FindFirstChild(trailName)
		if not flag then flag=Instance.new("BoolValue"); flag.Name=trailName; flag.Value=true; flag.Parent=owned end
...
purchaseEvent.OnServerEvent:Connect(function(player, action)
	...
	if typeof(productId) ~= "number" or productId <= 0 then purchaseEvent:FireClient(player,false,"Product ID belum dipasang: "..action); return end
	MarketplaceService:PromptProductPurchase(player, productId)
end)

trailEvent.OnServerEvent:Connect(function(player, trailName)
	if typeof(trailName) ~= "string" then return end
	local cfg, cash, data = Config.TRAILS[trailName], cashOf(player), dataOf(player)
	...
	if owned:FindFirstChild(trailName) then player:SetAttribute("EquippedTrail",trailName); return end
