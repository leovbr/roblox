...
local function claimOnce(player,name)
	...
	if not folder or folder:FindFirstChild(name) then return false end
...
local function grant(player,action)
	...
	if action=="MysticEggZone7" then
		...
		for _,brainrot in ipairs(brainrots:GetChildren())do
			if brainrot:GetAttribute("PassId")==Config.GAME_PASSES.MysticEggZone7 then return end
		end
		-- The pass guarantees its dragon even if the normal enclosure cap is full.
...
MarketplaceService.PromptGamePassPurchaseFinished:Connect(function(player,passId,purchased)
	...
	for action,id in pairs(Config.GAME_PASSES)do
		if id==passId then grant(player,action); break end
	end
end)
