...
local function claimFolder(player)
	...
	return data and data:FindFirstChild("PassClaims")
end

local function claimOnce(player, name)
	local folder = claimFolder(player)
...
local function highestOwned(player, prefix)
	...
	for action, id in pairs(Config.GAME_PASSES) do
		...
		if n and ownsPass(player, id) and n > highest then
			highest = n
		end
	end
	return highest
...
local function grant(player, action)
	...
	if level then
		...
		if speed then
			speed.Value = math.max(speed.Value, math.clamp(level, 1, #Config.TREADMILL_SPEEDS))
		end
		return
	end
	...
	if action == "InstantMoney1M" then
		...
		if cash and claimOnce(player, action) then
	...
	if action == "SpeedInstant1M" then
		local speed = data:FindFirstChild("SpeedLevel")
		if speed and claimOnce(player, action) then
			speed.Value = math.min(speed.Value + 1, #Config.TREADMILL_SPEEDS)
		end
		return
	end

	if action == "MysticEggZone7" then
		local brainrots = data:FindFirstChild("Brainrots")
		local slots = data:FindFirstChild("EggSlots")
		...
		for _, brainrot in ipairs(brainrots:GetChildren()) do
			if brainrot:GetAttribute("PassId") == Config.GAME_PASSES.MysticEggZone7 then
				return
			end
		end
		...
		brainrot.Name = "Mystic_Celestial_Pink_Dragon_Pass"
		brainrot.Value = cfg.creature
		brainrot:SetAttribute("Tier", cfg.tier)
		brainrot:SetAttribute("Income", cfg.income)
		brainrot:SetAttribute("Zone", cfg.zone)
		brainrot:SetAttribute("Habitat", Config.ZONES[cfg.zone].habitat)
		brainrot:SetAttribute("EggColor", "Mystic")
		brainrot:SetAttribute("SizeName", "SuperBig")
		brainrot:SetAttribute("DisplayScale", 1.35)
		brainrot:SetAttribute("PassId", Config.GAME_PASSES.MysticEggZone7)
...
local function applyAll(player)
	...
	if bestTreadmill > 0 then
		grant(player, "Treadmill" .. bestTreadmill)
	end

	local bestSpeed = highestOwned(player, "SpeedX")
	if bestSpeed > 0 then
		grant(player, "SpeedX" .. bestSpeed)
	end

	for action, id in pairs(Config.GAME_PASSES) do
		if not action:match("^Treadmill%d+$") and not action:match("^SpeedX%d+$") and ownsPass(player, id) then
			grant(player, action)
		end
	end
end
...
for _, player in ipairs(Players:GetPlayers()) do
	task.spawn(function() applyAll(player) end)
end
